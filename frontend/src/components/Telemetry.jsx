...
import { Building2, Pill, Truck, Clock } from 'lucide-react';
...
export default function Telemetry({ networkData }) {
  ...
  const { hospitals = [], ambulances = [], request_queue = [] } = networkData;
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0f1d]">
      {/* Sleek Top Pill Tabs (Simple English) */}
      <div className="flex items-center border-b border-slate-800/80 bg-slate-900/80 p-2 gap-1.5 sticky top-0 z-10">
        ...
        >
          ...
          <span>Ambulances ({activeAmbulancesCount}/{ambulances.length} Busy)</span>
        </button>
        ...
        {request_queue.length > 0 && (
          ...
          >
            ...
            <span>Waiting Calls ({request_queue.length})</span>
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {activeTab === 'fleet' && (
          <div className="space-y-3">
            ...
            {ambulances.map((amb) => {
              const isEnRoute = amb.status.startsWith('EN_ROUTE');
              return (
                <div
                  key={amb.id}
                  className={`bg-slate-900/80 border rounded-2xl p-3.5 space-y-2 transition-all shadow-sm ${
                    isEnRoute ? 'border-amber-500/40 bg-amber-950/10' : 'border-slate-800'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Truck className={`w-4 h-4 ${isEnRoute ? 'text-amber-400 animate-bounce' : 'text-slate-400'}`} />
                  ...
                  <div className="flex justify-between text-[11px] text-slate-400 font-mono">
                    <span>Number Plate: <strong className="text-slate-200">{amb.plate}</strong></span>
                  ...
                  {amb.assigned_request_id && (
                    <div className="pt-1.5 border-t border-slate-800/80 flex justify-between text-[10px] text-slate-400 font-mono">
                      ...
                      <span className="text-amber-300 font-bold">{amb.assigned_request_id}</span>
                    </div>
                  )}
                </div>
              );
        ...
        {activeTab === 'queue' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-rose-400 font-mono">
              <span>CALLS WAITING FOR AMBULANCE</span>
            ...
            {request_queue.map((req) => {
              ...
              return (
                <div
                  key={req.request_id}
                  className="bg-rose-950/20 border border-rose-500/40 rounded-2xl p-3.5 space-y-2"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-rose-200">{req.patient_name}</span>
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded ${tier.badge}`}>
                      Priority {req.urgency_tier}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-400 font-mono">
                    Village: <strong className="text-slate-200">{req.village_id}</strong> | Doctor: <strong className="text-slate-200">{req.specialty_needed}</strong>
                  </p>
                  <span className="text-[10px] font-mono text-amber-400 block">Waiting for the next available ambulance...</span>
                </div>
              );
            })}
          </div>
